...
describe ColissimoLabel::GenerateLabel do
  ...
  context 'Colissimo relay point label' do
    let(:relay_path) { './spec/data/relay/' }

    before do
      stub_request(:post, "https://ws.colissimo.fr/sls-ws/SlsServiceWSRest/2.0/generateLabel").
        to_return(status: 200, body: File.read(relay_path + 'generated_label'), headers: {})
    end
    ...
    it 'generates customs documents and Colissimo label' do
      ...
      expect(File.exist?(ColissimoLabel.colissimo_local_path + 'relay_label.pdf')).to be true
    end
  end
